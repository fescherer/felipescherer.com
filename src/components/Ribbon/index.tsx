...
export function Ribbon({ title, bgColor, textColor, href, className }: RibbonProps) {
  return (
    <Link href={href} className={cn('absolute -right-2 top-1 rounded rounded-ee-none px-2 text-sm leading-relaxed transition-all hover:brightness-75', className)} style={{ background: bgColor, color: textColor }}>
      <span>{title}</span>
